...
async function testV3SendPayload(destField) {
  try {
    const payload = {
      senderid: senderId,
    ...
    console.log('v3 Send Result for', JSON.stringify(destField), ':', text);
  } catch (err) {
...
async function run() {
  await testV3SendPayload({ destinations: [destination] });
  await testV3SendPayload({ destinations: destination });
  await testV3SendPayload({ recipient: [destination] });
}
