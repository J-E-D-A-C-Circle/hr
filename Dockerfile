...
CMD ["sh", "-c", "until nc -z tempstaff-db 3306 || nc -z db 3306; do echo 'Waiting for TempStaff MySQL database...'; sleep 2; done; npx prisma db push && node scripts/import_real_data.js && node scripts/update_bank_accounts.js && node .next/standalone/server.js"]
